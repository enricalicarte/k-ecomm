    ...
    <main className="bg-bone text-charcoal min-h-screen">
               ...
               {products.map((product) => (
                <Link href={`/${product.slug || product.id}`} key={product.id} className="group flex flex-col items-center cursor-pointer">
                  ...
                  <div className="pt-6 flex flex-col justify-between w-full text-center">
                    ...
                    <p className="text-charcoal/80 font-mono text-xs tracking-widest pt-2">{Number(product.price || 0).toFixed(2)} €</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="bg-charcoal text-bone px-6 md:px-12 py-24 text-center border-t border-charcoal/20">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
             <Link href="/lookbook" className="inline-block px-10 py-5 bg-transparent text-bone border border-bone/20 text-[10px] font-medium tracking-[0.2em] uppercase hover:bg-bone hover:text-charcoal transition-all duration-500 ease-out">
                View All Designers
             </Link>
